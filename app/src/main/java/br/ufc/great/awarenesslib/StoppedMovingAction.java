...
public class StoppedMovingAction implements FenceAction {
    public static final String TAG = "StoppedMovingAction";
    @Override
    public void doOperation(Context context, FenceState state) {
        if(state.getCurrentState() == FenceState.TRUE){
            ...
            values.put(ActivityRegisterContract.ActivityRegisterEntry.COLUMN_NAME_ACTIVITY, ActivityRegister.STOPPED);
